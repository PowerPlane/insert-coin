...
| **Contact scope** | `contacts` had only `duck_id`, `value`, `created`. | `scope` ∈ `david` / `keeper` / `keeper_and_david`, plus `epoch_id` so consent expires with the keeper it was given to. **"Nobody" is spelled *no row at all*** — nothing stored is nothing to leak. **REVISED:** the plan listed three options and the default was `keeper_and_david`; the contact screen says "Want David to reply?" and "Only David sees this", so that default stored consent WIDER than the sentence the person read. `david` was added and made the default. On David's own card all three collapse to the same reader — it only matters on the ninety-nine somebody else keeps. |
