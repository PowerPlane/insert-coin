...
| **2** | One real card | 🟡 **one card works** | Flashed, recorded, tapped — `&c=` matches character for character. A second card is the done-when. |
...
**Left to do: a second card.** "Two cards open two pages" would pass even
with the derivations disagreeing. Two cards whose recorded serials both
match their tapped URLs is the actual done-when.
