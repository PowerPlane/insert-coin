...
var DRAFT_KEY = "pond.intention-draft.v1";
...
var EN = {
  ...
  // Heading
  "arrival.03": "Keep this fortune as it is, or give it an intention for today.",
  // Body
  "arrival.04": "Set an intention",
  // Button
  "arrival.05": "Keep the fortune",
  // Button
  "studio.01": "Back",
  // Button
  ...
  "pond.01": "14 ducks",
  // Button
  "pond.02": "David's Pond — who made this. Opens davidyang.work in a new tab.",
  // Screen reader
  "pond.41": "David's Pond",
  // The mark on the water
...
function releaseFlow(opts) {
  ...
  function intentionBody() {
    root2.replaceChildren();
    const { root: viewRoot, body: wrap2 } = view();
    wrap2.append(nav({ label: t("studio.01"), onClick: arrival2 }, t("personal.01")));
    wrap2.append(preview(6), el("h2", "p-title", t("personal.02")));
    ...
    wrap2.append(intentionField.wrap, el("p", "p-note", t("personal.05")));
    const actions = el("div", "p-actions");
    actions.append(
  ...
  function intention() {
    screen(root2, intentionBody);
  }
  function saved(made) {
    screen(root2, () => savedBody(made));
  }
  const started = draft.intention || draft.studio.stickers.length || draft.studio.tint !== 0 || draft.studio.paint.some((value) => value !== 0);
...
async function pondScreen(bootstrap) {
  ...
  const mark = el2("a", "p-mark", t("pond.41"));
...
function openDuckCard(view2, duck) {
  ...
  if (duck.message) {
    card.append(
      el2("p", "p-eyebrow", t("personal.02")),
      el2("p", "p-card-msg", duck.message)
    );
  }
