...
import { NvmUpdi, type UpdiDevice } from "./nvm.js";
...
export class NvmUpdiP0 extends NvmUpdi {
  ...
  override async chipErase(): Promise<void> {
  ...
  override async eraseFlashPage(address: number): Promise<void> {
    if (!await this.waitNvmReady()) {
      throw new Error(
  ...
  override async eraseEeprom(): Promise<void> {
    if (!await this.waitNvmReady()) {
      throw new Error(
  ...
  override async eraseUserRow(address: number, size: number): Promise<void> {
    if (!await this.waitNvmReady()) {
      throw new Error(
  ...
  override async writeFlash(address: number, data: Uint8Array): Promise<void> {
    await this.writeNvm(address, data, true);
  }

  override async writeFlashErase(address: number, data: Uint8Array): Promise<void> {
    // ERWP: the page is erased and the page buffer written in one command,
    // so no chip erase is needed first. Added for the Insert Coin flasher.
    await this.writeNvm(address, data, true, NvmUpdiP0.NVMCMD_ERASE_WRITE_PAGE);
  }

  override async writeUserRow(address: number, data: Uint8Array): Promise<void> {
    // On this NVM variant user row is implemented as EEPROM
    await this.writeEeprom(address, data);
  }

  override async writeEeprom(address: number, data: Uint8Array): Promise<void> {
    await this.writeNvm(address, data, false, NvmUpdiP0.NVMCMD_ERASE_WRITE_PAGE);
  }

  override async writeFuse(address: number, data: Uint8Array): Promise<void> {
    if (!await this.waitNvmReady()) {
      throw new Error(
