...
export class UpdiApplication {
  ...
  async readDeviceInfo(): Promise<SibInfo | null> {
    ...
    if (sibInfo.NVM === "0") {
      ...
      // Only the P:0 controller (tinyAVR 0/1/2, megaAVR 0) is carried here.
      // The Dx/Ex drivers were dropped with the vendored copy; see VENDORED.md.
   ...
   */
  async writeFuse(address: number, value: number): Promise<void> {
    if (!this.nvm) {
      throw new Error('NVM driver not initialized');
    }
    await this.nvm.writeFuse(address, new Uint8Array([value & 0xff]));
  }
